...
# 未完工
@timeline_api_bp.route("/api/timeline/add-timeline", method = ["POST"])
def add_timeline():
    ...
    for und in unformated_durations:
        ...
        """
        try: formated_durations.append(duration(start_time = und[0], end_time = und[1]))
        except Exception as e: return jsonify({"status": "error", "message": e})
    if "description" in data: 
        description = data["description"]
        try: tl = timeline(name, formated_durations, description)
        except Exception as e: return jsonify({"status": "error", "message": e})
    else: 
        try: tl = timeline(name, formated_durations)
        except Exception as e: return jsonify({"status": "error", "message": e})
    org.timelines.append(tl)
    export_data_to_file(org_path, org)
    return jsonify({"status": "success", "data": tl.export_data()})
